...
def probability_prompt(action_name):
    return (
        f"For this decision, use the v3 simulation probability: you have a "
        f"{ACTION_PROBABILITY_PERCENT}% chance to {action_name} and a "
        f"{100 - ACTION_PROBABILITY_PERCENT}% chance to do nothing. "
        "Decide whether to act according to that probability and your persona. "
        "If you choose not to act, reply with 'None'.\n\n"
    )
...
def generate_simulation(n_steps,
    ...
    for i in range(start_iter, start_iter + n_steps):
        ...
        for agent in AGENT_LIST:
            ...
            if True:
                prompt_follow = (
                    probability_prompt("follow one user") +
                    "Here are the users that you already follow:\n" +
                    f"{getattr(agent, 'follow', set())}\n\n" +
            ...
            if True:
                prompt_retweet = (
                    probability_prompt("retweet one tweet") +
            ...
            if True:
                prompt_like = (
                    probability_prompt("like one tweet") +
            ...
            if True:
                prompt_comment = (
                    probability_prompt("comment on one tweet") +
            ...
            if True:
                prompt_organic = (
                    probability_prompt("write one new message") +
