...
from tools import *
from agent_v6_hashtag import *

# Count how many tweets so far
TWEET_ID_COUNTER = 0

ACTION_PROBABILITY = 0.50
ACTION_PROBABILITY_PERCENT = int(ACTION_PROBABILITY * 100)


def ssot_prompt(action_name):
    return (
        "Use String Seed of Thought (SSoT) for this probabilistic decision. "
        "First, generate a random alphanumeric seed string. Then manipulate that "
        "string deterministically, for example by using character codes and modular "
        "arithmetic, to obtain an integer from 0 to 99. "
        f"The v3 probability is {ACTION_PROBABILITY_PERCENT}%: if your integer is "
        f"less than {ACTION_PROBABILITY_PERCENT}, {action_name}; otherwise do nothing. "
        "After any brief SSoT trace, put your parser-ready answer after a line that "
        "starts with 'FINAL:'. If you do not act, use exactly 'FINAL: None'. If you "
        "act, use the exact response format requested below after the FINAL line.\n\n"
    )


def normalize_decision_response(response):
    lines = response.strip().splitlines()
    for index, line in enumerate(lines):
        if line.strip().lower().startswith("final:"):
            inline = line.split(":", 1)[1].strip()
            if inline:
                return "None" if inline.lower() == "none" else inline
            final_lines = [part.strip() for part in lines[index + 1:] if part.strip()]
            if not final_lines:
                return "None"
            if len(final_lines) == 1 and final_lines[0].lower() == "none":
                return "None"
            return "\n".join(final_lines)
...
def generate_simulation(n_steps,
    ...
    for i in range(start_iter, start_iter + n_steps):
        ...
        for agent in AGENT_LIST:
            ...
            # 3) always ask whether the agent wants to follow
            if True:
                prompt_follow = (
                    ssot_prompt("follow one user") +
                    "Here are the users that you already follow:\n" +
                    f"{getattr(agent, 'follow', set())}\n\n" +
                ...
                resp = normalize_decision_response(user_proxy.last_message()["content"].strip())
            ...
            # 4) always ask whether the agent wants to retweet
            if True:
                prompt_retweet = (
                    ssot_prompt("retweet one tweet") +
            ...
            # 5) always ask whether the agent wants to like
            if True:
                prompt_like = (
                    ssot_prompt("like one tweet") +
                ...
                resp_like = normalize_decision_response(user_proxy.last_message()["content"].strip())
            ...
            # 6) always ask whether the agent wants to comment
            if True:
                prompt_comment = (
                    ssot_prompt("comment on one tweet") +
                ...
                resp_comment = normalize_decision_response(user_proxy.last_message()["content"].strip())
            ...
            # 7) always ask whether the agent wants to generate 1 new message
            new_entries = []
            if True:
                prompt_organic = (
                    ssot_prompt("write one new message") +
                ...
                resp_new = normalize_decision_response(user_proxy.last_message()["content"].strip())
